...
// ДОБАВЛЯЕМ price И oldPrice К КАЖДОМУ ТОВАРУ
for (let i = 0; i < products.length; i++) {
  const rub = getRubPrices(products[i].usdPrice);
  products[i].price = rub.price;
  products[i].oldPrice = rub.oldPrice;
}
...
function renderFilterModal() {
  ...
  document.querySelectorAll(".filter-row").forEach(row => {
    row.addEventListener("click", () => {
      ...
      if (typeof window.showPage === 'function') window.showPage("catalog");
    });
  });
}

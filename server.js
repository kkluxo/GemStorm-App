...
const query = `CREATE TABLE IF NOT EXISTS orders ( id             SERIAL PRIMARY KEY, order_number   INTEGER, date           TEXT, time           TEXT, timestamp      TEXT, items          TEXT, total          INTEGER, status         TEXT DEFAULT 'Ожидание кода', status_code    TEXT DEFAULT 'pending', promo          TEXT, promo_discount INTEGER DEFAULT 0, payment_method TEXT, sender_name    TEXT, email          TEXT, user_id        BIGINT, user_name      TEXT, user_username  TEXT, verification_code TEXT, created_at     TIMESTAMPTZ DEFAULT NOW() )`;

try {
await pool.query(query);
console.log(‘Таблица orders готова’);
await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS verification_code TEXT`);
await pool.query(`CREATE TABLE IF NOT EXISTS referrals ( id SERIAL PRIMARY KEY, user_id BIGINT UNIQUE, referred_by BIGINT, created_at TIMESTAMPTZ DEFAULT NOW() )`);
await pool.query(`CREATE TABLE IF NOT EXISTS referral_balances ( id SERIAL PRIMARY KEY, user_id BIGINT UNIQUE, balance INTEGER DEFAULT 0 )`);

await pool.query(`CREATE TABLE IF NOT EXISTS referral_promocodes ( id SERIAL PRIMARY KEY, user_id BIGINT, code TEXT UNIQUE, amount INTEGER, used BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW() )`);

await pool.query(`CREATE TABLE IF NOT EXISTS referral_conversions ( id SERIAL PRIMARY KEY, user_id BIGINT, code TEXT, amount INTEGER, created_at TIMESTAMPTZ DEFAULT NOW() )`);
...
if (bonus > 0) {
await pool.query(`INSERT INTO referral_balances (user_id, balance) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET balance = referral_balances.balance + $2`, [referrerId, bonus]);
if (BOT_TOKEN) {
const bot = new Telegraf(BOT_TOKEN);
...
if (!userId) return res.json([]);
const result = await pool.query(
`SELECT rc.code, rc.amount, rc.created_at, rp.used  FROM referral_conversions rc LEFT JOIN referral_promocodes rp ON rp.code = rc.code WHERE rc.user_id = $1 ORDER BY rc.created_at DESC`,
[userId]
);
res.json(result.rows);
} catch(err) { res.status(500).json({ error: err.message }); }
...
try {
const result = await pool.query(`SELECT user_id, user_name, COALESCE(SUM(total), 0) as total_volume FROM orders WHERE status_code IN ('done', 'completed') AND user_id IS NOT NULL GROUP BY user_id, user_name ORDER BY total_volume DESC LIMIT 50`);
res.json(result.rows);
} catch(err) { res.status(500).json({ error: err.message }); }
});
...
try {
const result = await pool.query(‘SELECT * FROM reviews ORDER BY created_at DESC LIMIT 100’);
res.json(result.rows);
} catch(err) { res.status(500).json({ error: err.message }); }
});
...
try {
const bot = new Telegraf(BOT_TOKEN);
await bot.telegram.sendMessage(ADMIN_ID, `⭐ Новый отзыв от ${userName || 'Пользователь'}\nОценка: ${'★'.repeat(starsVal)}\n\n${text.trim()}`);
} catch(e) {}
}
res.json({ success: true });
} catch(err) { res.status(500).json({ error: err.message }); }

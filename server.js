...
// Ссылка на изображение для предпросмотра в каждом сообщении бота
const PREVIEW_IMAGE_URL = 'https://storage.botpapa.me/files/e89661a0-4591-11f1-bef9-f1ec7a2c6e45.jpeg';

// Невидимый символ со ссылкой — даёт предпросмотр изображения в Telegram
// Используем символ нулевой ширины (U+200B) внутри ссылки-заглушки
...
// Уведомление пользователя о создании заказа (новый формат)
    ...
    try {
        ...
        await bot.telegram.sendMessage(order.user_id, message, {
            ...
            reply_markup: {
                inline_keyboard: [[
                    {
                        text: '📦 Открыть заказ в приложении',
                        web_app: { url: ORDERS_PAGE_URL }
                    }
                ]]
            }
        });
                ...
                await ctx.reply(message, {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            ...
                            [
                                {
                                    text: '🛍 Открыть приложение GemStorm',
                                    web_app: { url: APP_URL }
                                }
                            ]
                        ]
...
app.post('/api/update-status', async (req, res) => {
    try {
        ...
        // Уведомление пользователя об обновлении статуса (новый формат)
        // Не отправляем уведомление если статус "Ожидает проверки"
...
app.post('/api/reviews', async (req, res) => {
    try {
        const { userId, userName, photoUrl, text, stars, orderId, orderNumber } = req.body;
        ...
        await pool.query(
            ...
            [userId || null, userName || 'Пользователь', photoUrl || null, text.trim(), starsVal, orderId || null, orderNumber || null]
        );
        ...
        if (bot) {
            try {
                ...
                console.error('Ошибка уведомления о отзыве:', e.message);
            }
        }
